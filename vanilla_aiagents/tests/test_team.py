...
class TestTeam(unittest.TestCase):
    ...
    def test_transitions(self):
        ...
        flow = Team(id="flow", description="", members=[first, second], llm=self.llm, allowed_transitions={first: [second]}, stop_callback=lambda x: len(x) > 3)
    ...
    def test_use_tools(self):
        ...
        flow = Team(id="flow", description="", members=[first, second], llm=self.llm, stop_callback=lambda x: len(x) > 3, include_tools_descriptions=True)
        workflow = Workflow(askable=flow)
        workflow.restart()
