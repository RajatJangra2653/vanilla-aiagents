...
class TestAgent(unittest.TestCase):
    ...
    def test_workflow_run_streaming(self):
        agent1 = Agent(id="agent1", llm=self.llm, description="Call this agent for general purpose questions", system_message = """You are an AI assistant
        ...
        team = Team(id="team", description="", members=[agent1, user], llm=self.llm, stop_callback=lambda msgs: len(msgs) > 4)
        workflow = Workflow(askable=team)
        
        stack_count = 0
        for mark, content  in workflow.run_stream("Which is expected delivery date for my order? Use format like February 1, 2022"):
            logging.debug(f"Mark: {mark}, Content: {content}")
            if mark == "start":
                stack_count += 1
                if stack_count == 1:
                    self.assertEqual(content, team.id, f"Expected {team.id} for start mark at stack count 1")
                if stack_count == 2:
                    self.assertEqual(content, agent1.id, f"Expected {agent1.id} for start mark at stack count 2")
            elif mark == "end":
                stack_count -= 1
                if stack_count == 1:
                    self.assertEqual(content, agent1.id, f"Expected {agent1.id} for end mark at stack count 1")
                if stack_count == 0:
                    self.assertEqual(content, team.id, f"Expected {team.id} for end mark at stack count 0")
                    break
            elif mark == "response":
                self.assertIn("January 1, 2022", content[0]["content"], "Expected delivery date not found in the response")
        
        self.assertIn("January 1, 2022", workflow.conversation.messages[-1]["content"], "Expected delivery date not found in the conversation messages")
